...
| OQ-P2-5 | Tiles relocate to `<src>/tiles/` per spec §2. Resolved 2026-07-07: confirmed — config.yaml and Ansible serving-path changes land in the same deploy; one-time manual cleanup of old `<src>/<ts>/` dirs on production. **Garganorn repo change implemented, pending merge** (slug-aware `/tiles/<slug>/<path>` route, `serve_dir` removed, `config.yaml` updated to Phase 2 paths; 931 passed/1 xfailed). Ansible infra wiring deferred to when tiles go to prod. | pending merge | — |
