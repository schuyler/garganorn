"""The atgeo containment level vocabulary (phase2b-design.md Part A, OQ-P2-2).
...
it is never used as an input to this mapping (phase2b-design.md §A.3).
...
Two consumers derive from this dict so SQL and Python cannot drift:
  - garganorn/stages.py renders a `${level_case}` CASE expression (with NO
    ELSE branch, §A.6 belt-and-braces) into overture_division_import.sql
    from LEVEL_VOCAB.
...
Values follow a uniform stride-5 (§A.3): borough=55, macrohood=60,
neighborhood=65, microhood=70. This is a protocol change from the narrower
§1.7 table (which only defined levels through neighborhood=60) — macrohood
and microhood are additions, and neighborhood moves from 60 to 65.
...
def level_case_sql(subtype_expr: str = "d.subtype", alias: str | None = None) -> str:
    ...
    `level` from a division `subtype` column (phase2b-design.md §A.6). The
